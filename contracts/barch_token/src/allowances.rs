use cosmwasm_std::{
    ...
    Storage, Uint128, CosmosMsg, WasmMsg, to_binary,
};
use cw20::{AllowanceResponse, Cw20ReceiveMsg, Expiration};

use crate::error::ContractError;
use crate::state::{ALLOWANCES, BALANCES, TOKEN_INFO, CONFIG, RewardContractResponseMsg};
...
pub fn execute_burn_from(
    ...
    let config = CONFIG.load(deps.storage)?;
    let mut messages:Vec<CosmosMsg> = vec![];
    let decrease_balance_from_owner= RewardContractResponseMsg::DecreaseBalance{
        address: owner.clone(),
        amount
    };
    messages.push(CosmosMsg::Wasm(WasmMsg::Execute {
        contract_addr: config.reward_contract.clone(),
        msg: to_binary(&decrease_balance_from_owner)?,
        funds: vec![],
    }));
    //***************************************************************
...
pub fn execute_send_from(
    ...
    let res = Response::new().add_message(msg).add_attributes(attrs).add_messages(messages);
    Ok(res)
}
...
mod tests {
    ...
    use cosmwasm_std::testing::{mock_dependencies_with_balance, mockenv, mock_info};
