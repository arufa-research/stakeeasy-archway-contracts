...
/**
 * Update seArch token addr in config
 */
pub fn try_update_se_arch_addr(
    deps: DepsMut,
    _env: Env,
    info: MessageInfo,
    address: Addr,
    ...
    if info.sender != config.admin {
        return Err(ContractError::Std(StdError::generic_err(
            "Only admin can update seArch Address"
        )));
    }

    config.seArch_token = Some(address);
...
/**
 * Update barch token addr in config
 */
pub fn try_update_barch_addr(
    ...
    if info.sender != config.admin {
        return Err(ContractError::Std(StdError::generic_err(
            "Only admin can update barch Address"
        )));
    }

    config.barch_token = Some(address);
    CONFIG.save(deps.storage, &config)?;
