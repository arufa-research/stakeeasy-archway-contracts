...
use crate::deposit::{try_claim_stake, try_stake, try_stake_for_barch};
use crate::error::ContractError;
use crate::msg::{ExecuteMsg, InstantiateMsg, QueryMsg, RewardClaim, MigrateMsg};
use crate::query::{
    query_active_undelegation, query_barch_exchange_rate, query_current_window,
    query_dev_fee, query_info, query_pending_claims, query_seArch_exchange_rate,
...
use crate::state::{State, STATE, BARCH_FROZEN_TOTAL_ONCHAIN, BARCH_FROZEN_TOKENS, SEARCH_FROZEN_TOTAL_ONCHAIN, SEARCH_FROZEN_TOKENS};
...
use crate::update::{
    try_update_barch_addr, try_update_rewards_addr, try_update_se_arch_addr,
...
const CONTRACT_NAME: &str = "stake-easy-archway";
...
pub fn instantiate(
    ...
    // store state
    let state = State {
        seArch_backing: Uint128::from(0u128),
        barch_backing: Uint128::from(0u128),
        to_deposit: Uint128::from(0u128), // amount of arch in contract (not validators)
        not_redeemed: Uint128::from(0u128),
        barch_under_withdraw: Uint128::from(0u128), // amount of arch under 28 days withdraw
        seArch_under_withdraw: Uint128::from(0u128), // amount of seArch under 28 days withdraw
        arch_under_withdraw: Uint128::from(0u128),
        seArch_to_burn: Uint128::from(0u128),
        barch_to_burn: Uint128::from(0u128),
    };
...
pub fn execute(
    ...
    match _msg {
        ...
        ExecuteMsg::StakeForbarch { referral } => try_stake_for_barch(deps, env, _info, _msg, referral),

        // claim rewards and stakes arch from contract to validators
        ExecuteMsg::ClaimAndStake {} => try_claim_stake(deps, env, _info, _msg),

        ExecuteMsg::UpdateSearchAddr { address } =>
            try_update_se_arch_addr(deps, env, _info, address),
        ExecuteMsg::UpdateBarchAddr { address } => 
            try_update_barch_addr(deps, env, _info, address),
        ...
        // init withdraw arch from validator to contract after 21 days and advance to next window
        ExecuteMsg::AdvanceWindow {} => advance_window(deps, env, _info, _msg),
        // for user to transfer arch from contract to self wallet if 21 day window is complete
...
pub fn try_remove_data(deps: DepsMut, _env: Env, info: MessageInfo, window: u64) -> Result<Response, ContractError> {
    ...
    for (user_addr, user_arch_amount) in matured_amounts?.iter() {
        if *user_arch_amount == Uint128::from(0u128) {
...
pub fn try_remove_old_claim(deps: DepsMut, _env: Env, info: MessageInfo) -> Result<Response, ContractError> {
    ...
    for (user_addr, user_arch_amount) in claim_amounts?.iter() {
        if *user_arch_amount == Uint128::from(0u128) {
...
pub fn try_remove_old_queue_data(deps: DepsMut, _env: Env, info: MessageInfo) -> Result<Response, ContractError> {
    ...
    for (user_addr, user_arch_amount) in queue_amounts?.iter() {
        if *user_arch_amount == Uint128::from(0u128) {
    ...
    for (user_addr, user_arch_amount) in bqueue_amounts?.iter() {
        if *user_arch_amount == Uint128::from(0u128) {
...
pub fn try_kill_switch_unbond(
    ...
    // Freeze barch rate
    let barch_token = config.barch_token.clone().ok_or_else(|| {
    ...
    let tokens =
        query_total_supply(deps.querier, &barch_token)?
        .saturating_sub(state.barch_to_burn);
    let total = Uint128::from(total_on_chain);
    BARCH_FROZEN_TOTAL_ONCHAIN.save(deps.storage, &total)?;
    BARCH_FROZEN_TOKENS.save(deps.storage, &tokens)?;

    // Freeze seArch rate
    let seArch_token = config.seArch_token.clone().ok_or_else(|| {
        StdError::generic_err(
    ...
    let tokens_se =
        query_total_supply(deps.querier, &seArch_token)?
        .saturating_sub(state.seArch_to_burn);
    let total_se = Uint128::from(total_on_chain_se);
    SEARCH_FROZEN_TOTAL_ONCHAIN.save(deps.storage, &total_se)?;
    SEARCH_FROZEN_TOKENS.save(deps.storage, &tokens_se)?;
...
pub fn try_redelegate(
    ...
    if let Some(validator) = removed {
        ...
        if val_rewards.u128() > 0 {
            ...
            state.to_deposit += val_rewards.saturating_sub(Uint128::from(barch_reward));
            state.seArch_backing += val_rewards.saturating_sub(Uint128::from(barch_reward));
        }
...
pub fn query(deps: Deps, _env: Env, _msg: QueryMsg) -> StdResult<Binary> {
    match _msg {
        ...
        QueryMsg::seArchExchangeRate {} => query_seArch_exchange_rate(deps),
        QueryMsg::BArchExchangeRate {} => query_barch_exchange_rate(deps),
