...
pub fn execute(
    ...
    match msg {
        ...
        ExecuteMsg::UpdateBarchAddr {address} => {
            try_update_barch_addr(deps, env, info, address)
        }
...
pub fn try_update_barch_addr(
    ...
    config.barch_contract = Some(deps.api.addr_canonicalize(&address)?);
    store_config(deps.storage, &config)?;
