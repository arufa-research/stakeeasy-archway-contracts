...
pub fn try_claim_simple_rewards(
    ...
    // It would not overflow at any time as per total supply of arch coins
...
//only barch contract should increase or decrease (or add any account to the reward contract)
pub fn try_increase_balance(
    ...
    if config.barch_contract == None {
        return Err(error::ContractError::Std(StdError::generic_err(
            "barch not registered"
        )));
    }else {
        if let Some(cannonical) = config.barch_contract {
            token_address = deps.api.addr_humanize(&cannonical)?;
        }
    }
    // let token_address = deps.api.addr_humanize(&barch_addr)?;
