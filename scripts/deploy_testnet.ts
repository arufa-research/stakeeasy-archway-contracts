...
import { SearchTokenContract } from "../artifacts/typescript_schema/SearchTokenContract";
import { BarchTokenContract } from "../artifacts/typescript_schema/BarchTokenContract";
...
async function run () {
  ...
  const staking_contract_info = await staking_contract.instantiate(
    {
      ...
      "dev_fee": 5000,  // 5%
      "epoch_period": 3600*24*7 + 2*60,  // 4 days + 2 mins in seconds
      "underlying_coin_denom": "uconst",
      "unbonding_period": 3600*24*21 + 2*60, // 28 days + 2 mins in seconds
      "reward_denom": "uconst",
      "er_threshold": 970,
      "peg_recovery_fee": 10,
    },
  ...
  const deposit_res = await staking_contract.stake(
    {
      account: contract_owner,
      transferAmount: [{"denom": "uconst", "amount": "1000"}] ,// 25 archx
      customFees:
        {
          amount: [{ amount: "2000", denom: "uconst" }],
          gas: "1000000",
  ...
  const deposit_res_barch = await staking_contract.stakeForbarch(
    {
      account: contract_owner,
      transferAmount: [{"denom": "uconst", "amount": "2000"}], // 15 archx
      customFees: {
